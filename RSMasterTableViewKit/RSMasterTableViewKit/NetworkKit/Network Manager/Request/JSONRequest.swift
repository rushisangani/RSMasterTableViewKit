...
/// JSONRequest
public class JSONRequest: RequestProtocol {
    ...
    /// execute request
    /// returns JSON
    /// specify response keypath to get JSON for specific path i.e. "data", "data.users", "data.content.message"
    public func execute(responseKeyPath: String? = nil,
                        completion: @escaping (Result<Response, ResponseError>) -> ()) {
        NetworkManager.shared.requestJSON(self, responseKeyPath: responseKeyPath, completion: completion)
    }
