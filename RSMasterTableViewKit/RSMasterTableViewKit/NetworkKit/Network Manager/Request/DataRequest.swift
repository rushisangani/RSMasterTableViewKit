...
/// DataRequest
public class DataRequest: RequestProtocol {
    ...
    /// execute request
    /// returns Data
    /// specify response keypath to get Data for specific path i.e. "data", "data.users", "data.content.message"
    public func execute(responseKeyPath: String? = nil,
                        completion: @escaping (Result<Response, ResponseError>) -> ()) {
        NetworkManager.shared.requestData(self, responseKeyPath: responseKeyPath, completion: completion)
    }
